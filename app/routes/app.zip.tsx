...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  return {
    ...
    eligible: eligible.map(({ order, originalZip, recommendedZip }) => ({
      id: order.id,
      name: order.name,
      originalZip,
      recommendedZip,
            ...
            <div style={{ marginTop: 24 }}>
              <s-table>
                <s-table-header-row>
                  ...
                  <s-table-header>Current ZIP</s-table-header>
                  <s-table-header>Recommended ZIP</s-table-header>
                </s-table-header-row>
                <s-table-body>
                  {scanResult.eligible.map((o) => (
                    <s-table-row key={o.id}>
                      ...
                      <s-table-cell>{o.originalZip}</s-table-cell>
                      <s-table-cell>{o.recommendedZip}</s-table-cell>
                    </s-table-row>
                  ))}
                </s-table-body>
              </s-table>
